test_that("All LSS implementations produce equivalent results", {
  ...
  beta_optimized <- lss_optimized(Y = Y, bdes = bdes, use_cpp = FALSE)
  ...
  expect_equal(dim(beta_optimized), c(n_trials, n_voxels))
  ...
  expect_equal(beta_optimized, beta_r, tolerance = 1e-8,
               info = "Optimized R should be equivalent to original R implementation")
})
...
test_that("LSS implementations handle edge cases correctly", {
  ...
  beta_optimized_single <- lss_optimized(Y = Y_single, bdes = bdes_single, use_cpp = FALSE)
  
  expect_equal(beta_r_single, beta_cpp_single, tolerance = 1e-8)
  expect_equal(beta_r_single, beta_naive_single, tolerance = 1e-6)
  expect_equal(beta_optimized_single, beta_r_single, tolerance = 1e-8)
  ...
  beta_optimized_fixed <- lss_optimized(Y = Y_single, bdes = bdes_fixed, use_cpp = FALSE)
  
  expect_equal(beta_r_fixed, beta_cpp_fixed, tolerance = 1e-8)
  expect_equal(beta_r_fixed, beta_naive_fixed, tolerance = 1e-6)
  expect_equal(beta_optimized_fixed, beta_r_fixed, tolerance = 1e-8)
})
