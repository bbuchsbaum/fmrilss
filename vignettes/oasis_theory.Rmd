...
Optimized Analytic Single-pass Inverse Solution (OASIS) extends Least Squares Separate (LSS) estimation with a set of algebraic shortcuts and HRF-aware features. This vignette documents the math behind those shortcuts and ties each derivation back to the implementation in `R/oasis_glue.R` and `src/oasis_core.cpp`. Basic regression facts (QR projection, ridge, etc.) are assumed knowledge; the focus here is on OASIS-specific components.
...
We first treat the single-basis case (one regressor per trial) before generalising to multi-basis HRFs.
...
OASIS packages the common pieces (projections, norms, cross-products) so every trial can be solved from shared ingredients.
...
Let \(n_{jv} = \langle a_j, RY_{\cdot v} \rangle\) and \(m_v = \langle s, RY_{\cdot v} \rangle\). The pair \((\beta_j, \gamma_j)\) solving the 2×2 system for trial \(j\) and voxel \(v\) is obtained from
...
where \(B_{jv} \in \mathbb{R}^K\). `oasisk_betas()` solves this 2K×2K system via Cholesky factorisation. Ridge again adds \(\lambda_x I\) and \(\lambda_b I\) to the block diagonals. Compared to the single-basis path, only the shapes of the cached matrices differ; the solve is still analytic per trial/voxel block.
...
- Closed-form solves: \(\mathcal{O}(N V)\) with negligible constants (2×2 or 2K×2K systems)
