...
SBHM gives you **trial-by-trial activation amplitudes** for each voxel, where each voxel's HRF shape is:

1. **Library-constrained**: Comes from a predefined set of physiologically plausible HRFs (e.g., gamma functions with varying peak times and widths)
2. **Voxel-specific**: Each voxel gets its own best-matching HRF from the library
3. **Interpretable**: You know exactly which HRF parameters (peak time, width) each voxel uses
4. **Efficiently estimated**: Uses only r=4-8 shared basis functions instead of fitting full HRF timecourses

**Concrete output:**
- `amplitude`: (ntrials × V) matrix of trial-wise activation strengths per voxel
- `matched_idx`: Which library HRF each voxel matched to (e.g., "voxel 42 uses gamma HRF with peak=5.2s")
- `margin`: Confidence in the match (high margin = unambiguous HRF assignment)
- `coeffs_r`: Low-dimensional (r) trial-wise coefficients in the shared basis (optional)

**Key advantage:** SBHM scales to large libraries (50-200+ candidate HRFs) while fitting only r parameters per voxel, making voxel-specific HRF modeling practical for whole-brain analysis.
...
### Recommended entry point: `lss_sbhm_design()`

If you already use `fmridesign`, prefer `lss_sbhm_design()` to avoid manually
building an OASIS `design_spec`. This mirrors `lss_design()` while running the
SBHM pipeline end-to-end.

```{r sbhm-design-entrypoint, eval=FALSE}
if (requireNamespace("fmridesign", quietly = TRUE)) {
  library(fmridesign)

  # trials: data.frame with at least onset + run (run-relative onsets)
  emod <- event_model(
  ...
  res <- lss_sbhm_design(
    ...
    sbhm = sbhm,
    event_model = emod,
    # baseline_model = bmodel,  # optional drift/block/nuisance terms
...
**Note:** If you use the factorized prepass (`prepass = list(data_fac = ...)`), the
prepass stage intentionally skips prewhitening for efficiency. You can still
enable prewhitening for the final OASIS step by passing `prewhiten = list(...)`
to `lss_sbhm()`.

### Basic SBHM Run

```{r run-sbhm, eval=eval_chunks}
# Minimal end-to-end SBHM: specify only what you want to override.
# (All control lists are optional, and you can provide partial lists.)
res_sbhm <- lss_sbhm(
  Y = Y,
  sbhm = sbhm,
  design_spec = design_spec,
  # match = list(topK = 3, soft_blend = TRUE),     # optional: blend ambiguous voxels
  # prewhiten = list(method="ar", p=1L, exact_first="ar1"),  # optional: short TR
...
Note on top-K: by default `lss_sbhm()` uses hard assignment (top-1) even if you
request `topK > 1`. To *blend* the top-K candidates for ambiguous voxels, use
`match = list(topK = K, soft_blend = TRUE)` (optionally with `blend_margin` to
blend only when the margin is small). If you need custom weighting (e.g., a
different softmax temperature), use the manual pipeline with `sbhm_prepass()`,
`sbhm_match(topK = K)`, and `sbhm_project()`.
...
| **prepass.ridge** | NULL (off) | 0.005 to 0.05 | Enable for noisy / ill-conditioned designs |
...
# Uses: topK=1, whiten=TRUE, orient_ref=TRUE, no prewhitening.
# (Prepass ridge is off by default; enable it via prepass = list(ridge = ...).)
```
...
- Project: amplitudes = projection of trial-wise r‑coefficients onto `alpha_soft` using per‑trial Gram weighting (a_j' a_j):
  a_hat = (alpha' D_j beta_j) / (alpha' D_j alpha), where D_j = A_j' A_j for trial j.

Note on orientation: before projection, we align each voxel’s matched
coordinates to the average direction of its trial-wise coefficients to avoid
arbitrary sign flips (basis sign is not identifiable). This improves amplitude
interpretability and cross-voxel comparability.
...
## Performance and Prewhitening

### Why SBHM’s Prepass Avoids Full Trial Matrices

The `sbhm_prepass()` step now builds the aggregated per-basis design directly
via `fmrihrf::regressor(..., summate = TRUE)` rather than constructing the
full trial-wise matrix and summing its columns. This yields the same
aggregated design `A_agg` (T×r) with significantly lower memory and runtime when
the number of trials is large.

Practical effects:
- Memory scales with `T×r` instead of `T×(r·ntrials)` during prepass.
- The subsequent ridge-regularized solve is unchanged; default fractional ridge
  (`lambda = 0.01`) stabilizes the `G = A' A` system.

### Prewhitening in the Factorized Path (by design)

When providing factorized data (`data_fac = list(scores, loadings)`),
`sbhm_prepass()` intentionally skips prewhitening. Applying fmriAR-based
whitening directly in the factorized representation would require estimating
and applying per-parcel/run filters consistently to both `Scores` and the
aggregated design, which defeats the purpose of the low‑memory factorized path.

Recommended pattern:
- Use the factorized prepass for shape-only matching efficiency.
- Apply prewhitening in the final OASIS step, where `lss_sbhm(..., prewhiten = list(...))`
  forwards options to `lss(method = "oasis")`. This yields whitened trial-wise
  coefficients and amplitudes, even if the prepass was factorized.

### Benchmark: SBHM vs Voxel-wise HRF

We ship an illustrative benchmark script. It simulates data, runs
`lss_sbhm()` end-to-end, and compares timing against a voxel-wise HRF workflow
(`estimate_voxel_hrf()` + `lss_with_hrf()`). Results will vary by machine and
library/basis choice, but SBHM typically shows substantial speedups.

Run from an R session after installing/loading `fmrilss`:

```r
path <- system.file("benchmarks", "bench_sbhm.R", package = "fmrilss")
if (nzchar(path)) source(path) else message("Benchmark script not found in installed package.")
```

Notes:
- The script prefers an FIR basis if available for the voxel-wise comparator,
  otherwise falls back to SPMG3.
- To adapt for your data, adjust `T`, `V`, trials, rank `r`, and HRF library.

## Amplitude Policy

The final amplitude stage estimates per‑trial scalars from the matched HRF shape per voxel. Different regimes benefit from different estimators:

- global_ls (default): single‑shape GLM across all trials with fractional ridge (e.g., 0.02 of mean diag(X'X)). Use for slow or moderately overlapped ER designs. Works well with optional AR prewhitening (`prewhiten`).
- lss1: per‑trial 2×2 LSS (trial vs. others) with fractional ridge on both arms. Gentler under strong overlap (ISI 1–4 s) and typically more stable when trial columns are highly collinear.
- oasis_voxel: runs OASIS with K=1 per voxel using the matched HRF columns, returning SEs/t‑stats. Heavier but most consistent; recommended as a fallback in extreme overlap.

You can select the method and add a per‑voxel fallback when conditioning is poor:

```r
out <- lss_sbhm(
  Y, sbhm, design_spec,
  prewhiten = list(method = "ar", p = "auto", pooling = "run", exact_first = "ar1"),
  amplitude = list(
    method = "global_ls",
    ridge  = list(mode = "fractional", lambda = 0.02),
    ridge_frac = list(x = 0.02, b = 0.02),
    cond_gate  = list(metric = "rho", thr = 0.999, fallback = "lss1"),
    return_se = FALSE   # set TRUE to compute per-trial SEs
  )
)

table(out$diag$method_used)
summary(out$diag$rho_max)
```

Guidance by regime:

- Slow ER (≥12 s): global_ls, fractional ridge 0.01–0.02, prewhitening optional.
- Moderate ER (4–8 s): global_ls with ridge 0.02, prewhitening recommended; fallback to lss1 for voxels with rho > 0.999.
- Fast ER (ISI 1–4 s): global_ls with ridge 0.02–0.05 and prewhitening; enable cond_gate fallback to lss1 or oasis_voxel. Soft‑blend (`match$topK>1`) can modestly help matching under ambiguity.

### ISI Sweep (Example)

The snippet below sweeps two regimes and reports method usage and conditioning. It runs quickly in fast mode.

```r
set.seed(101)
Tlen <- if (fast_mode) 160 else 220
V    <- if (fast_mode)  6 else  24
r    <- 4
sframe <- fmrihrf::sampling_frame(blocklens = Tlen, TR = TR)

# Small HRF library and SBHM basis
H <- cbind(
...
simulate_design <- function(isi, jitter) {
  bounds <- c(10, Tlen - 30)
  ...
  list(sframe = sframe, cond = list(onsets = sort(base), duration = 0, span = 24))
}

simulate_Y <- function(design_spec, noise_sd = 0.12) {
...
spec_slow <- simulate_design(isi = 15, jitter = 2)
spec_fast <- simulate_design(isi = 2,  jitter = 0.5)
Y_slow <- simulate_Y(spec_slow)
Y_fast <- simulate_Y(spec_fast)

out_slow <- lss_sbhm(
  Y_slow, sbhm, spec_slow,
  prewhiten = list(method = "ar", p = "auto", pooling = "run"),
  amplitude = list(method = "global_ls", ridge = list(mode = "fractional", lambda = 0.02))
)

out_fast <- lss_sbhm(
  Y_fast, sbhm, spec_fast,
  prewhiten = list(method = "ar", p = "auto", pooling = "run"),
  amplitude = list(method = "global_ls",
                   ridge = list(mode = "fractional", lambda = 0.02),
                   cond_gate = list(metric = "kappa", kappa_thr = 50, fallback = "lss1"))
)

cat("Slow ER: method usage\n"); print(table(out_slow$diag$method_used))
cat("Fast ER: method usage\n"); print(table(out_fast$diag$method_used))
summary(out_fast$diag$kappa)
```

Interpretation:
- Slow ER: most voxels use the fast global_ls path.
- Fast ER: some voxels auto‑fallback to lss1 based on conditioning; amplitudes remain stable and diagnostics transparent.

### SEs and t‑stats with oasis_voxel

When the fallback (or selected) method is `oasis_voxel`, per‑trial SEs are available by design, and returned in `out$diag$se` as an `ntrials × V` matrix (NA elsewhere). This makes it easy to compute t‑statistics and masks.

```r
# Force oasis_voxel for demonstration
out_ov <- lss_sbhm(
  Y_fast, sbhm, spec_fast,
  prewhiten = list(method = "ar", p = "auto", pooling = "run"),
  amplitude = list(method = "oasis_voxel", ridge_frac = list(x = 0.02, b = 0.02)),
  return = "amplitude"
)

se_mat <- out_ov$diag$se              # ntrials × V (NA if method != oasis_voxel)
t_mat  <- out_ov$amplitude / se_mat   # elementwise; NA propagated where se is NA

# Example: count voxels with at least one |t| > 2 across trials
hit_vox <- colSums(abs(t_mat) > 2, na.rm = TRUE)
summary(hit_vox)
```

If you prefer to use `global_ls` as default and only fallback to `oasis_voxel` in extreme cases, `out$diag$se` will be NA for non‑oasis voxels by design—mask by `out$diag$method_used == "oasis_voxel"` when aggregating.

### Auto‑tuning thresholds and adaptive ridge (optional)

You can use the observed diagnostics to set a reasonable gate and to adapt ridge per voxel:

```r
# Auto-set gate to the 99th percentile of kappa
kappa_thr <- fmrilss:::`.sbhm_auto_gate_threshold`(out_fast$diag$kappa, metric = "kappa", q = 0.99, hard_min = 1000)

# Adaptive fractional ridge suggestion based on kappa (clip to [0.02, 0.08])
lam_vec <- fmrilss:::`.sbhm_adaptive_ridge_gls`(out_fast$diag$kappa, base = 0.02, k0 = 1000, max_lam = 0.08)
summary(lam_vec)
```

These helpers give you principled starting points for `cond_gate` and per‑voxel ridge in fast ER designs.
