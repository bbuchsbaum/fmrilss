...
Standard GLM analysis (Least Squares All, LSA) estimates all trial coefficients simultaneously by solving:

$$\boldsymbol{\beta} = (\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}'\mathbf{Y}$$

When trials occur in rapid succession, the design matrix columns exhibit high correlation (condition number > 30), producing unstable trial estimates with inflated variance.
...
$$\boldsymbol{\beta}_i = (\mathbf{X}_i'\mathbf{X}_i)^{-1}\mathbf{X}_i'\mathbf{Y}$$

where $\mathbf{X}_i = [\mathbf{x}_i \mid \sum_{j \neq i} \mathbf{x}_j \mid \mathbf{Z} \mid \mathbf{N}]$, with $\mathbf{x}_i$ the trial regressor, $\mathbf{Z}$ experimental covariates, and $\mathbf{N}$ nuisance regressors.
...
The design matrix encodes stimulus timing and expected hemodynamic responses. When available, we recommend building event‑related designs with fmridesign::event_model(), which cleanly specifies HRF bases, blocks/runs, and parametric modulators. We include a backward‑compatible fallback if the package is not installed.

```{r design}
# Define sampling frame and trial onsets (in TRs)
sframe <- fmrihrf::sampling_frame(blocklens = n_timepoints, TR = 1)
# Spread trials across the run but induce overlap with jitter
bounds <- c(10L, n_timepoints - 24L)
base_onsets <- round(seq(from = bounds[1], to = bounds[2], length.out = n_trials))
set.seed(42)
# Slightly more overlap via larger jitter window (±3 TR)
jitter <- sample.int(7, n_trials, replace = TRUE) - 4L  # -3..+3 TR
onsets <- sort(pmax(bounds[1], pmin(base_onsets + jitter, bounds[2])))

if (requireNamespace("fmridesign", quietly = TRUE)) {
  # Build one HRF column per trial directly with fmridesign
  des <- data.frame(onset = onsets, run = 1, trial = factor(seq_len(n_trials)))
  hrf <- fmridesign::hrf  # bind symbol for formula
  emod <- fmridesign::event_model(
    onset ~ hrf(trial, basis = "spmg1"),
    data = des, block = ~run, sampling_frame = sframe, precision = 0.1
  )
  X <- fmridesign::design_matrix(emod)
  # Ensure numeric matrix downstream
  if (inherits(X, "Matrix")) X <- as.matrix(X)
  if (!is.matrix(X)) X <- as.matrix(X)
} else {
  # Fallback: simple HRF‑convolved design using a gamma kernel (one column per trial)
  simple_hrf <- function(t) ifelse(t >= 0, dgamma(t, shape = 6, rate = 1), 0)
  X <- matrix(0, n_timepoints, n_trials)
  hrf_kernel <- simple_hrf(0:20)  # HRF spanning ~20 TRs
  for (i in seq_len(n_trials)) {
    onset <- onsets[i]
    hrf_length <- min(length(hrf_kernel), n_timepoints - onset + 1)
    X[onset:(onset + hrf_length - 1), i] <- hrf_kernel[1:hrf_length]
  }
}

# Ensure downstream dimensions use the actual number of trial columns in X
n_trials_eff <- ncol(X)

# Experimental regressors (Z) — intercept and linear trend
# These are regressors we want to model but not trial‑wise
...
Matrix `X` contains HRF-convolved trial regressors. Matrix `Z` contains experimental covariates (intercept and linear trend). The nuisance matrix models unwanted variance (e.g., motion parameters).
...
```{r plot-trial-regressors, fig.width=8, fig.height=4, echo=FALSE}
...
true_trial_betas <- matrix(rnorm(n_trials_eff * n_voxels, 0, 1.2), n_trials_eff, n_voxels)
...
# Voxel-specific AR parameters (slower but more accurate)
# Default pooling="global" estimates one AR model across all voxels
# pooling="voxel" fits separate AR models per voxel (~100x slower)
beta_voxel <- lss(Y, X, Z = Z, Nuisance = Nuisance,
                  prewhiten = list(method = "ar", p = 1, pooling = "voxel"))

# ARMA model for complex noise (AR + moving average components)
# Use when residuals show both autocorrelation and moving average patterns
beta_arma <- lss(Y, X, Z = Z, Nuisance = Nuisance,
                prewhiten = list(method = "arma", p = 2, q = 1))
```

**Pooling strategies:**
- `pooling = "global"` (default): Single AR model for all voxels (fast)
- `pooling = "voxel"`: Per-voxel AR models (accurate but slow)
- `pooling = "run"`: Separate model per run (for multi-run data)
...
Before comparing beta histograms, it helps to see the trial-wise design that drives identifiability. The heatmap below shows the HRF‑convolved trial regressors (columns of `X`), and the right panel shows their column‑wise correlations. Highly overlapped designs (bright off‑diagonal blocks) tend to inflate variance in LSA and motivate LSS.

```{r design-visualization, fig.width=8, fig.height=4.5, fig.alt="Heatmap of trial-wise design X and the corresponding column correlation matrix."}
par(mfrow = c(1, 2), mar = c(4, 4, 2, 1))

# Heatmap of X (time x trials)
image(t(X[nrow(X):1, , drop = FALSE]), axes = FALSE,
      main = "Trial-wise design X",
      col = gray.colors(64, start = 1, end = 0))
axis(1, at = seq(0, 1, length.out = ncol(X)), labels = seq_len(ncol(X)), cex.axis = 0.7)
lab_y <- pretty(1:nrow(X), n = 5)
axis(2, at = seq(0, 1, length.out = length(lab_y)), labels = lab_y, las = 1, cex.axis = 0.7)
box()

# Column correlation matrix
CX <- suppressWarnings(cor(X))
image(CX[ncol(X):1, , drop = FALSE], axes = FALSE,
      main = "Correlation across trials",
      col = gray.colors(64, start = 1, end = 0))
axis(1, at = seq(0, 1, length.out = ncol(X)), labels = seq_len(ncol(X)), cex.axis = 0.7)
axis(2, at = seq(0, 1, length.out = ncol(X)), labels = rev(seq_len(ncol(X))), las = 1, cex.axis = 0.7)
box()

# Report a simple conditioning summary
sv <- svd(X, nu = 0, nv = 0)$d
cond_num <- if (length(sv) > 1) max(sv) / max(min(sv), .Machine$double.eps) else 1
cat(sprintf("\nDesign summary: trials = %d, condition number ≈ %.1f\n", ncol(X), cond_num))
```

## LSS versus Traditional GLM: When Each Shines

Comparison between LSS and standard Least Squares All (LSA) methods. LSA is the traditional GLM approach that estimates all trial coefficients simultaneously in a single model, making it computationally faster but more susceptible to collinearity in rapid designs.

```{r lsa-comparison, fig.alt="Beta distributions for LSS vs LSA (example voxel), with variance comparison across voxels."}
# LSA: Standard GLM (Least Squares All) estimates all trials simultaneously
# This is computationally faster but suffers from collinearity in rapid designs
...
# Plot comparison (single voxel example; same x-range for both histograms)
par(mfrow = c(1, 2))
v <- 1L
xlim1 <- range(c(beta_full[, v], beta_lsa[, v]))
hist(beta_full[, v], main = "LSS: Trial betas (voxel 1)",
     xlab = "Beta values", col = "lightblue", xlim = xlim1)
hist(beta_lsa[, v], main = "LSA: Trial betas (voxel 1)",
     xlab = "Beta values", col = "lightgreen", xlim = xlim1)
```

### Scatter: LSS vs LSA (shrinkage visualization)

The scatter below compares trial-wise betas from LSS (y-axis) against LSA (x-axis). The dashed gray line shows identity; the red line is an LM fit through the origin (LSS ~ LSA). With greater temporal overlap, LSS typically shows mild shrinkage relative to LSA.

```{r lss-lsa-scatter, fig.width=6.5, fig.height=5.2}
df_sc <- data.frame(LSA = as.vector(beta_lsa), LSS = as.vector(beta_full))
if (requireNamespace("ggplot2", quietly = TRUE)) {
  rng <- range(df_sc$LSA, df_sc$LSS)
  ggplot2::ggplot(df_sc, ggplot2::aes(LSA, LSS)) +
    ...
    ggplot2::labs(title = "Trial-wise betas: LSS vs LSA",
                  subtitle = "Dashed: identity; Red: LSS ~ LSA slope (through origin)",
                  x = "LSA beta", y = "LSS beta") +
    ggplot2::theme_minimal(base_size = 12)
} else {
  op <- par(no.readonly = TRUE); on.exit(par(op), add = TRUE)
  plot(df_sc$LSA, df_sc$LSS, pch = 19, col = rgb(0.17, 0.5, 0.72, 0.35),
       xlab = "LSA beta", ylab = "LSS beta", main = "Trial-wise betas: LSS vs LSA")
  abline(0, 1, lty = 2, col = "gray50")
  coef1 <- coef(lm(LSS ~ 0 + LSA, data = df_sc))[1]
  abline(0, coef1, col = "#d7301f", lwd = 2)
}
```
...
```{r oasis-intro}
# Basic OASIS usage with automatic design construction
# fmrihrf is now imported automatically
beta_oasis <- lss(
  Y = Y,
  X = NULL,  # Let OASIS build design from design_spec
  method = "oasis",
  oasis = list(
    design_spec = list(
      ...
      cond = list(
        ...
        hrf = HRF_SPMG1,        # SPM canonical HRF
        span = 30               # HRF duration in seconds
      )
    ),
    ridge_mode = "fractional",  # Regularization for numerical stability
    ridge_x = 0.01,             # Ridge for design matrix
    ridge_b = 0.01              # Ridge for coefficients
  )
)

cat("OASIS beta dimensions:", dim(beta_oasis), "\n")

# Alternative: Provide a pre-built design matrix directly
# beta_oasis_manual <- lss(Y, X = my_hrf_convolved_design, method = "oasis", ...)
```

**Why `X = NULL`?** OASIS can build the design matrix internally from `design_spec`, ensuring the HRF settings stay synchronized with the design. This is especially useful when exploring different HRFs or using multi-basis models. Alternatively, you can provide your own HRF-convolved design via `X`.

## Working with Different Computational Backends

The package provides multiple backends to match your computational needs and resources. Each backend implements the same algorithm but with different optimization strategies. The `naive` implementation offers the clearest code for understanding the algorithm, while `r_vectorized` and `r_optimized` provide good performance with pure R code. For production work with large datasets, `cpp_optimized` offers the best performance, especially when combined with parallel processing.
...
# Ground truth depends on which design matrix we use:
# - When fitting with X (unmodulated):        recovers modulator * true_trial_betas
# - When fitting with X_param (premodulated): recovers true_trial_betas directly
...
Choosing the right backend for your data size is crucial for optimal performance:

**Performance guidelines (based on benchmarks):**

| Data Size | Recommended Method | Rationale |
|-----------|-------------------|-----------|
| n_trials < 50, n_voxels < 5,000 | `r_optimized` or `cpp_optimized` | Similar speed, R easier to debug |
| n_voxels > 10,000 | `cpp_optimized` | Benefits from OpenMP parallelization |
| Need HRF grid search or ridge | `oasis` | Additional features justify overhead |
| Quick prototyping | `r_optimized` | Good balance of speed and readability |

In practice, pick the backend that matches your needs: R for debugging, C++ for maximum throughput, OASIS for HRF-aware workflows. Always validate with a quick benchmark on your own data.
...
# Check for collinearity in design matrix
cor_matrix <- cor(X)
...
if (nrow(high_cor) > 0) {
  ...
  # Option 1: Use OASIS with ridge and automatic design construction
  beta_ridge <- lss(
    Y = Y,
    X = NULL,  # Let OASIS build design
    method = "oasis",
    oasis = list(
      design_spec = list(
        sframe = sampling_frame(blocklens = nrow(Y), TR = 1),
      ...
      ridge_mode = "absolute",
      ridge_x = 0.1  # Stronger ridge for collinearity
    )
  )

  # Option 2: For standard LSS without OASIS, increase ISI or use prewhitening
  # Ridge is not directly available for non-OASIS backends
}
```
...
mem_Y <- object.size(Y)
mem_designs_approx <- mem_Y * n_trials  # n_trials copies of design during LSS
mem_results <- mem_Y  # Output size similar to Y
mem_required <- mem_Y + mem_designs_approx + mem_results

cat("Estimated memory needed:", round(mem_required / 1e9, 2), "GB\n")

# If this exceeds available RAM, use chunking approach (see example above)
# Or consider OASIS which has lower memory overhead for large datasets
```
...
To deepen your understanding and explore advanced features, we recommend:

- **Voxel-wise HRF vignette**: Model spatial variation in hemodynamic responses across the brain using fully flexible HRF estimation
- **SBHM vignette**: Learn efficient voxel-specific HRF estimation using Shared-Basis HRF Matching, a library-constrained approach that balances flexibility and computational efficiency
- **OASIS method vignette**: Comprehensive coverage of OASIS features including automatic HRF construction, ridge regularization, and multi-basis models
- **Mixed models vignette**: Combine LSS with mixed-effects modeling frameworks for hierarchical analyses
