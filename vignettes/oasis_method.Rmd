...
Enter OASIS—Optimized Acquisition and Stimulus Sequencing—a method that reimagines how we compute LSS estimates. Rather than iterating through trials one by one, OASIS leverages clever mathematical reformulations to compute all trial estimates simultaneously in a single, highly optimized pass. But OASIS is more than just a speed improvement; it brings sophisticated features like automatic HRF estimation, ridge regularization for numerical stability, and seamless handling of multi-basis HRF models that would be cumbersome to implement in traditional frameworks.

This vignette assumes you're comfortable with the basic LSS concepts from our getting started guide, understand HRF models and basis functions from working with neuroimaging data, have some familiarity with ridge regression and regularization concepts, and know the basics of the `fmrihrf` package for HRF modeling. With this foundation, we'll explore how OASIS can transform your approach to trial-wise beta estimation.
...
The FIR approach reveals the actual shape of the hemodynamic response without parametric constraints. While this flexibility is powerful, it comes at the cost of many more parameters to estimate, making regularization especially important.
...
sizes <- list(
  small = c(200, 100),   # 200 timepoints, 100 voxels
  medium = c(300, 500),  # 300 timepoints, 500 voxels
  large = c(400, 2000)   # 400 timepoints, 2000 voxels
)

benchmark_results <- data.frame()

for (size_name in names(sizes)) {
  n_time <- sizes[[size_name]][1]
  n_vox <- sizes[[size_name]][2]

  # Create test data
  Y_bench <- matrix(rnorm(n_time * n_vox), n_time, n_vox)

  # Create sampling frame and onsets for this size
  sframe_bench <- sampling_frame(blocklens = n_time, TR = TR)
  onsets_bench <- seq(10, n_time - 20, length.out = min(20, floor(n_time/15)))

  # Standard LSS with pre-built design
  dm_bench <- design_matrix(
    sframe = sframe_bench,
    conditions = list(list(onsets = onsets_bench, hrf = HRF_SPMG1)),
    tr_per_trial = TRUE
  )$X
  ...
  # Time standard LSS (C++ optimized) if available
  time_cpp <- NA
  ...
  benchmark_results <- rbind(benchmark_results, data.frame(
    ...
    Timepoints = n_time,
    Voxels = n_vox,
    Trials = length(onsets_bench),
    R_Optimized = round(time_r_opt, 3),
    CPP_Optimized = round(time_cpp, 3),
    OASIS = round(time_oasis, 3),
    Speedup_vs_R = round(time_r_opt / time_oasis, 1),
    Speedup_vs_CPP = if (!is.na(time_cpp)) round(time_cpp / time_oasis, 1) else NA
  ))
}
...
plot(benchmark_results$Voxels, benchmark_results$R_Optimized,
     ...
     ylim = c(0, max(benchmark_results$R_Optimized, na.rm = TRUE)))
lines(benchmark_results$Voxels, benchmark_results$OASIS,
      type = "b", col = "red", pch = 17)
if (any(!is.na(benchmark_results$CPP_Optimized))) {
  lines(benchmark_results$Voxels, benchmark_results$CPP_Optimized,
        type = "b", col = "green", pch = 15)
  legend("topleft", c("R Optimized", "C++ Optimized", "OASIS"),
         col = c("blue", "green", "red"), pch = c(19, 15, 17), lty = 1)
} else {
  legend("topleft", c("R Optimized", "OASIS"),
...
These benchmarks demonstrate OASIS's computational advantages, which stem from its mathematical reformulation that eliminates redundant calculations. The speedup factor typically increases with dataset size due to the efficiency of the closed-form solution approach.

The performance advantage is most pronounced when dealing with rapid event-related designs where the mathematical structure of LSS can be fully exploited. While exact speedup factors depend on dataset characteristics and hardware, OASIS consistently outperforms iterative approaches by avoiding redundant computations.
