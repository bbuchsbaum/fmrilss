...
stopifnot(ridge_summary$MeanTrialVariance[2] < ridge_summary$MeanTrialVariance[1])
```
...
#> Mean |t|: 2.77
stopifnot(
  all(dim(res_se$beta) == dim(res_se$se)),
  all(is.finite(res_se$se))
)
```
...
&nbsp;
    ...
    #> [46] glue_1.8.0          fmriAR_0.3.1        numDeriv_2016.8-1.1
