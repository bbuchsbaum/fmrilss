...
#'
#' Given trial-wise coefficients in the shared basis (r×ntrials×V) and the
#' voxel-specific matched library coordinates `alpha_hat` (r×V), compute scalar
#' amplitudes per trial and voxel via inner products.
#'
#' @param beta_rt 3D array of shape r × ntrials × V containing per-trial
#'   coefficients in the SBHM basis (as returned by OASIS with K=r, reshaped).
#' @param alpha_hat Numeric matrix r×V of matched library coordinates per voxel
...
#'
#' @return Numeric matrix ntrials × V of scalar amplitudes.
#'
#' @examples
#' \dontrun{
#'   # beta_mat returned by lss(..., method="oasis") with K=r has (r*ntrials) × V rows
#'   r <- nrow(alpha_hat)
...
sbhm_project <- function(beta_rt, alpha_hat) {
  ...
  if (nrow(alpha_hat) != r || ncol(alpha_hat) != V) {
    stop("alpha_hat must be r×V to match beta_rt dims")
  }
